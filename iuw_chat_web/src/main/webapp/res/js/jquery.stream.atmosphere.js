...
(function($) {
	...
	$.stream.setup({
		...
		// Atmosphere runs on any Java based web server maintaining session
		// using JSESSIONID cookie
		// and sets 'Access-Control-Allow-Origin' header to '*' unless
		// 'enableAccessControl' option is not false
		enableXDR : true,
		...
		handleOpen : function(text, message) {
			...
			// message.index = text.indexOf("<!-- EOD -->") + "<!-- EOD
			// -->".length;
			message.index = text.indexOf("<!-- EOD -->") + 12;
		},

		// This plugin assumes that you can send message in accord with the
		// default message format
		// if not possible, you can parse the response chunks directly by
		// implementing the following handler
